...
# 2. Add ILogRepository
content = content.replace('IOptions<AzureDevOpsOptions> devOpsOptions,', 'ILogRepository logRepository,\n        IOptions<AzureDevOpsOptions> devOpsOptions,')
...
# 4. Add Log properties
log_props = """
    public System.Collections.ObjectModel.ObservableCollection<AzureDevOps.Core.Interfaces.SystemLogEntity> CurrentLogs { get; } = new();
...
# 5. Update Load/Save config
content = content.replace('BaseUrl = _devOpsOptions.BaseUrl;', 'BaseUrl = _devOpsOptions.BaseUrl;\n        UseDefaultCredentials = _devOpsOptions.Auth.UseDefaultCredentials;\n        AuthDomain = _devOpsOptions.Auth.Domain ?? "";\n        AuthUsername = _devOpsOptions.Auth.Username ?? "";\n        AuthPassword = _devOpsOptions.Auth.Password ?? "";')
...
content = re.sub(r'var configJson = System.Text.Json.JsonSerializer.Serialize\(new\s*\{.*?\}\);\s*await _configurationRepository.SetConfigurationAsync\("SystemConfig", configJson, ct\);', save_repl, content, flags=re.DOTALL)

# 6. Update TestTfsConnectionAsync
...
"""
content = content.replace('public event PropertyChangedEventHandler? PropertyChanged;', log_methods + '\n    public event PropertyChangedEventHandler? PropertyChanged;')
